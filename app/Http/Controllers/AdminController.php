...
class AdminController extends Controller
{
    ...
    public function desVerify($id)
    {
        ...
        // Verificar si el usuario autenticado es admin y está intentando desverificarse a sí mismo
        if ($authUser->role === 'admin' && $authUser->id === $user->id) {
            return response()->json(['message' => 'No puedes desverificarte a ti mismo.'], 403);
        }

        // Verificar si el usuario a desverificar es superAdmin
        if ($user->role === 'superAdmin') {
            return response()->json(['message' => 'No puedes desverificar a un superAdmin.'], 403);
        }

        // Desverificar el correo electrónico del usuario
